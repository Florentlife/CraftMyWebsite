	...
	if($_Joueur_['rang'] == 1) {
		...
		for($i = 2; $i <= end($lastGrade); $i++) {
			if(file_exists($dirGrades.$i.'.yml')) {
				...
				$editGrade['PermsPanel']['info']['stats']['members']['showIP'] = $_POST['permsPanelInfoStatsMembersShowIP'.$i];
				...
				$editGrade['PermsPanel']['forum']['actions']['addPrefix'] = $_POST['permsPanelForumActionsAddPrefix'.$i];
				$editGrade['PermsPanel']['forum']['actions']['seePrefix'] = $_POST['permsPanelForumActionsSeePrefix'.$i];
				...
				$editGrade['PermsPanel']['upload']['manage'] = $_POST['permsPanelUploadManage'.$i];
